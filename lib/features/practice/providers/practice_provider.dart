...
import 'package:prompt_loop/domain/entities/streak.dart';
import 'package:prompt_loop/domain/entities/struggle_entry.dart';
...
final completedTasksCountProvider = FutureProvider.family<int, int>((
  ref,
  skillId,
) async {
  final repository = await ref.watch(practiceRepositoryProvider.future);
...
final totalTasksCountProvider = FutureProvider.family<int, int>((
  ref,
  skillId,
) async {
  final repository = await ref.watch(practiceRepositoryProvider.future);
...
final skillProgressPercentProvider = FutureProvider.family<double, int>((
  ref,
  skillId,
) async {
  final repository = await ref.watch(practiceRepositoryProvider.future);
...
final dailyProgressDataProvider =
    FutureProvider.family<
      List<Map<String, dynamic>>,
      ({int skillId, int daysBack})
    >((ref, params) async {
      final repository = await ref.watch(practiceRepositoryProvider.future);
...
final completedTasksForDayProvider =
    FutureProvider.family<int, ({int skillId, DateTime date})>((
      ref,
      params,
    ) async {
      final repository = await ref.watch(practiceRepositoryProvider.future);
      return repository.getCompletedTasksCountForDay(
        params.skillId,
        params.date,
      );
    });
